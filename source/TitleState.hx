...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			FlxTransitionableState.defaultTransIn = new TransitionData(FADE, FlxColor.GREEN, 1, new FlxPoint(0, -1), {asset: diamond, width: 32, height: 32},
				new FlxRect(-300, -300, FlxG.width * 1.8, FlxG.height * 1.8));
			FlxTransitionableState.defaultTransOut = new TransitionData(FADE, FlxColor.GREEN, 0.7, new FlxPoint(0, 1),
		...
		if (titleJSON.backgroundSprite != null && titleJSON.backgroundSprite.length > 0 && titleJSON.backgroundSprite != "none"){
			bg.loadGraphic(Paths.image('titlestuff/flippymenu1'));
		}else{
		...
		logoBl.y = FlxG.height * 550;
		logoBl.updateHitbox();
		logoBl.screenCenter();
		// logoBl.color = FlxColor.GREEN;
		...
		bg = new FlxSprite().loadGraphic(Paths.image('tittlestuff/flippytitle-' + FlxG.random.int(1, 4)));
		...
		//bgScroll = new FlxBackdrop(Paths.image('titlestuff/flippymenu1'), 0, 0, true, true);
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				//FlxTween.tween(FlxG.camera, {zoom: 0.7}, 3, {ease: FlxEase.expoInOut, type: BACKWARD});
				...
				new FlxTimer().start(1, function(tmr:FlxTimer)
				{
					FlxTween.tween(FlxG.camera, {zoom: 4}, 2, {ease: FlxEase.expoInOut, type: PINGPONG});
				});
				...
				{	
					...
					//FlxTween.tween(FlxG.camera, {zoom: 2}, 3, {ease: FlxEase.expoOut, type: BACKWARD});
					FlxTween.tween(black, {alpha: 1}, 0.3, {onComplete:
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 5:
					#if PSYCH_WATERMARKS
					createCoolText(['Not Ass.', 'With'], -40);
					#else
					createCoolText(['Psych Engine', 'Modified By'], -40);
					#end
				case 7:
					addMoreText('Happy', -40);
					ngSpr.visible = false;
				// credTextShit.text += '\nNewgrounds';
				case 8:
					deleteCoolText();
				...
				// credTextShit.text += '\nBAKA';
				case 15:
					addMoreText('Flippy'); // credTextShit.text += '\nENGINE';
				case 16:
					skipIntro();
			}
		}
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			else //Default! Edit this one!!
			{
				...
				FlxTween.tween(logoBl, {y: logoBl.y + 100}, 2, {ease: FlxEase.quadInOut, type: PINGPONG});
